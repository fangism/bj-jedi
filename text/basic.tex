...
\noindent
Let $p_X$ be the probability of drawing card $X$ to form a newly paired hand, 
and $q_X=1-p_X$.
...
The outcomes of $P^\prime$ can be approximated with replacement
(effectively infinite deck) as:
\begin{eqnarray}
...
& & +\Prob{P^\prime=P+1}\ExpOpt{XX,S-1,P+1,N} \\
\ExpOpt{XX,S,P,N} &=& \Max{\ExpAct{hit}{XX},\ExpAct{stand}{XX},\ExpAct{double}{XX},\ExpAct{split}{XX,S,P,N}}
\end{eqnarray}

\TODO: show some examples of expanded expressions

\noindent
When $S=0$, no more splits are allowed:
\begin{eqnarray}
\ExpAct{split}{XX,0,P,N} &=& P_n \ExpOpt{XX} +N \ExpOpt{XY}\\
\ExpOpt{XY} &=& \Sum{j}{\neq{}X}{\Prob{c=j} \ExpOpt{\Trans{X,j}}}\\
